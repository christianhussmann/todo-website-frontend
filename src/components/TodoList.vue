...
export default class todoList extends Vue{
  ...
  async loadTodos(){
    const response = await this.axios.get('http://localhost:8080/todos');
    ...
    async updateTodo(todo: Todo){
      console.log("Todo", todo);
      await this.axios.put('http://localhost:8080/todos(${todo.id}', {
        id: todo.id,
        title: todo.title,
        completed: todo.completed
      });
    ...
    async handleDelete(todo: Todo){
        await this.axios.delete('http://localhost:8080/todos(${todo.id}');
      ElMessage({
